...
public class MainTest {
	...
	static {
		try {
			cl = getClassLoaderFromPath(new File(CLASSPATH_DIR), Thread.currentThread().getContextClassLoader());
	...
	private static ClassLoader getClassLoaderFromPath(File path, ClassLoader parent) throws Exception {
		...
		File[] jarFiles = path.listFiles(new FileFilter() {
			public boolean accept(File file) {
				return file.getName().endsWith(MainTest.LIB_EXT);
			}
		});
	...
	public static void main(String[] args) throws Exception {
		Launcher launcher = Launcher.class.cast(Class.forName(LAUNCHER_CLASS, true, cl).newInstance());
		URLClassPath bootstrapClassPath = launcher.getBootstrapClassPath();
		//((Object) launcher).launch(new Object[] { "this string is capitalized" });
	}
}
